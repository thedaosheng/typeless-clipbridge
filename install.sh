...
for base in $BASE_URLS; do
  ...
  if command -v curl >/dev/null 2>&1; then
    if curl -fsSL --connect-timeout 8 --retry 1 "${base}/scripts/tcb-install.sh" -o "$TMP_FILE" 2>/dev/null; then
      export TCB_BASE_URL="$base"
      break
    fi
  elif wget -q --timeout=8 -O "$TMP_FILE" "${base}/scripts/tcb-install.sh" 2>/dev/null; then
    export TCB_BASE_URL="$base"
    break
  fi
done
...
if [[ ! -s "$TMP_FILE" ]]; then
  echo "typeless-clipbridge: failed to download scripts/tcb-install.sh from all mirrors." >&2
  exit 1
fi
